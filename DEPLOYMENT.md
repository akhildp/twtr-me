...
If your VPS uses UFW (Uncomplicated Firewall), you should allow web traffic.
...
## 7. Enable HTTPS (SSL)
The application is configured to use **Caddy** as a reverse proxy. This allows for automatic SSL certificates and hides the internal application port (3000).
...
Edit the `Caddyfile` to match your domain name:
```bash
nano Caddyfile
# Change "twtr.me" to your actual domain
```

### 3. Open Web Ports (IONOS Panel)
Ensure ports **80** and **443** are open in your **IONOS Cloud Panel** (Infrastructure -> Firewall Policies). Port 3000 is no longer needed externally.
...
## 8. Verify Deployment
-   **Check status**: `docker ps` (You should see `twtr-me` and `caddy`)
